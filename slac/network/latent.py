...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim=3, tactile_dim=6, img_dim=256, tactile_latent_dim=96, hidden_img=256):
        ...
        self.tactile_recognize = nn.Sequential(nn.Linear(tactile_latent_dim+hidden_img,(tactile_latent_dim+hidden_img)//2),
                                               nn.ReLU(),
                                               nn.Linear((tactile_latent_dim+hidden_img)//2,1),
                                               nn.Sigmoid()
                                               ).apply(self._init_weights)
        self.alignment_recognize = nn.Sequential(nn.Linear(tactile_latent_dim+hidden_img,(tactile_latent_dim+hidden_img)//2),
                                               nn.ReLU(),
                                               nn.Linear((tactile_latent_dim+hidden_img)//2,1),
                                               nn.Sigmoid()
                                               ).apply(self._init_weights)

        self.bottle_neck = nn.Sequential(nn.Linear(tactile_latent_dim+hidden_img,(tactile_latent_dim+hidden_img)*2),
                                         nn.ReLU(),
                                         nn.Linear((tactile_latent_dim+hidden_img)*2,(tactile_latent_dim+img_dim))).apply(self._init_weights)
